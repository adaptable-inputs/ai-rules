...
- MUST include stable context keys: `timestamp`, `level`, `service`, `component`, `operation`, `traceId`
...
- `DEBUG`: developer diagnostics that are safe but too noisy for production defaults.
...
- `FATAL` (if supported): unrecoverable startup/runtime failure before process termination.

Level selection rules:
- MUST NOT log expected domain outcomes as `ERROR` (for example validation failures caused by user input).
- MUST NOT log the same exception at multiple layers unless each layer adds distinct value.
- If an exception is rethrown unchanged, MUST log it at one boundary only.

## Event Design Rules
- SHOULD prefer event names that describe domain intent (`invoice.paid`, `auth.login.failed`) over technical noise.
...
- MUST NOT log secrets: passwords, API keys, tokens, private keys, session secrets, connection strings with credentials.
- MUST NOT log full personal data payloads; log minimal identifiers only when needed for supportability and allowed by
  policy.
...
- SHOULD avoid unbounded/high-cardinality fields in indexed logs: raw user input, full URLs with query strings, full
  stack traces as labels, random IDs as metric labels.
